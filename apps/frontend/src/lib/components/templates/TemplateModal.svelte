...
import { CategoryDropdown, Modal } from '$lib/components/ui';
...
let expectedQuadrant = $state<'green' | 'yellow' | 'red' | 'blue' | undefined>(
	undefined,
);
...
function parseDuration(str: string): number | undefined {
	...
	if (!match) {
		...
		return Number.isNaN(num) ? undefined : num * 60; // Assume minutes
	}
	let total = 0;
	if (match[2]?.toLowerCase().startsWith('h')) {
		total += Number.parseInt(match[1]) * 3600;
		if (match[3]) {
			total += Number.parseInt(match[3]) * 60;
		}
	} else {
		total += Number.parseInt(match[1]) * 60;
	}
	return total;
}
