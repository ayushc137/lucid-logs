...
export const parsers = {
    string: (defaultVal = '') => (val: string | null) => val ?? defaultVal,
    number: (defaultVal = 0) => (val: string | null) => val ? Number(val) : defaultVal,
    boolean: (defaultVal = false) => (val: string | null) => val === 'true',
    date: (defaultVal: Date = new Date()) => (val: string | null) => val ? new Date(val) : defaultVal,
    /**
     ...
     */
    dateOnly: (defaultVal: Date = new Date()) => (val: string | null) => {
        if (!val) return defaultVal;
        const [y, m, d] = val.split('-').map(Number);
        if (!y || !m || !d) return defaultVal;
        return new Date(y, m - 1, d);
    },
    array: (defaultVal: string[] = []) => (val: string | null) => val ? val.split(',') : defaultVal,
};
